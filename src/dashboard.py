...
# --- PAGE CONFIG ---
st.set_page_config(
    page_title="서울시 1인가구 상권 분석 대시보드",
    page_icon="🏙️",
    layout="wide",
...
def load_data():
    ...
    # 2. 소비
    spend_file = os.path.join(base_dir, '서울시 상권분석서비스(소비-행정동).csv')
    ...
    # 3. 점포
    store_file = os.path.join(base_dir, '서울시 상권분석서비스(점포-행정동)_2024년.csv')
    ...
    # 4. 추정매출 (2024년 데이터, Github 용량 제한으로 zip 압축 파일 사용)
    sales_file = os.path.join(base_dir, '서울시 상권분석서비스(추정매출-행정동)_2024년.zip')
    ...
    # 5. 길단위인구(유동인구)
    pop_walk_file = os.path.join(base_dir, '서울시 상권분석서비스(길단위인구-행정동).csv')
    try:
        df_walk = pd.read_csv(pop_walk_file, encoding='cp949')
    ...
    # 자치구명 결측치 보정 (소상공인 데이터와 행안부 데이터의 동이름 차이로 누락될 수 있는 부분 보완)
...
if menu == "1. 배후 수요 파악 (밀집도)":
    ...
    fig = px.bar(gu_agg, x='자치구명', y='총_1인가구수', color='총_1인가구수', color_continuous_scale='Blues', text_auto='.2s')
